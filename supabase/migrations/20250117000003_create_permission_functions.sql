-- Migration: Create permission functions for user permission system
-- Created: 2025-01-17
-- Description: Creates functions to check user permissions for menu items
...
BEGIN
  ...
  role_based_perms AS (
    ...
    AND rp.menu_item_id NOT IN (SELECT menu_item_id FROM user_specific_perms)
  )
...
COMMENT ON FUNCTION get_user_accessible_menu_items(UUID) IS 'Returns all menu items with access status for a specific user';
