...
interface TarefasStatusBoardProps {
  totalOpen: number
  totalOverdue: number
  items: TarefaOpenItem[]
}
...
function getDueLabel(item: TarefaOpenItem) {
  if (!item.dueDate) return 'Sem vencimento'
  if (item.isOverdue) return `Atrasada ${item.overdueDays ?? 0} dias`
  return 'No prazo'
}

export function TarefasStatusBoard({
  totalOpen,
  totalOverdue,
  items,
}: TarefasStatusBoardProps) {
  const router = useRouter()
  const [responsibleFilter, setResponsibleFilter] = useState('all')
  const [selectedStatus, setSelectedStatus] = useState<string | null>(null)

  const responsibleOptions = useMemo(
    () => Array.from(new Set(items.map((item) => item.responsibleName).filter(Boolean))).sort((a, b) => a.localeCompare(b)),
    [items],
  )
          ...
          <div className="space-y-3">
            {statusRows.length ? (
              ...
              <div className="rounded-2xl bg-[hsl(var(--surface-tint))] px-4 py-6 text-sm text-muted-foreground">
                Nenhuma tarefa pendente encontrada para este filtro.
              </div>
            )}
          </div>
        </div>
      ...
      <Dialog open={Boolean(selectedStatus)} onOpenChange={(open) => !open && setSelectedStatus(null)}>
        <DialogContent className="max-w-3xl rounded-[28px] border-white bg-white p-0">
          <DialogHeader className="border-b border-border/60 px-6 py-5">
            <DialogTitle className="sr-only">Tarefas no status {selectedStatus}</DialogTitle>
            ...
            <DialogDescription>
              {modalItems.length} tarefas encontradas para {selectedResponsibleLabel.toLowerCase()}.
            </DialogDescription>
          </DialogHeader>
