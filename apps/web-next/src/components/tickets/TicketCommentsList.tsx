...
type CommentSource = TicketComment | AtaComment;

type CommentWithAttachments = CommentSource & {
          ...
          <CardContent className="p-4">
            <div className="flex gap-3">
              <Avatar className="w-8 h-8 flex-shrink-0">
                <AvatarFallback 
                  style={{ backgroundColor: comment.author?.color || generateUserColor(comment.author?.id || comment.created_by || 'desconhecido', comment.author?.name || comment.created_by || 'Usuário') }}
                  className="text-white text-sm font-medium"
                >
                  {getUserInitials(comment.author?.name || comment.created_by || 'Usuário')}
                </AvatarFallback>
              </Avatar>
                ...
                {comment.attachments && comment.attachments.length > 0 && (
                  <div className="space-y-2">
                    <div className="flex flex-wrap gap-2">
                      {comment.attachments.map((attachment: TicketAttachment) => (
                        <div key={attachment.id} className="flex items-center gap-2 bg-muted rounded-md px-2 py-1">
                          ...
                          <div className="flex flex-col">
                            ...
                            {typeof attachment.size_kb === 'number' && (
                              <span className="text-xs text-muted-foreground">
                                {formatFileSize(attachment.size_kb ?? undefined)}
                              </span>
                            )}
                          </div>
                          <Button
