...
import { Rescisao, RescisaoPrioridade } from '@/repositories/rescisaoRepo.supabase';
...
type RescisaoTableRow = Omit<Rescisao, 'dueDate' | 'assignedTo'> & {
...
export default function RescissoesPage() {
  ...
  useEffect(() => {
    ...
    loadSecretarios();
    loadTags();
  }, [loadRescisoes, loadStatuses, loadSecretarios, loadTags]);
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        'postgres_changes',
        { event: 'INSERT', schema: 'public', table: 'omnia_rescisoes' },
        (payload) => {
          logger.debug('Nova rescisão criada:', payload);
          loadRescisoes();
        }
      )
      .on(
        'postgres_changes',
        { event: 'UPDATE', schema: 'public', table: 'omnia_rescisoes' },
        (payload) => {
          logger.debug('Rescisão atualizada:', payload);
          loadRescisoes();
        }
      )
      .on(
        'postgres_changes',
        { event: 'DELETE', schema: 'public', table: 'omnia_rescisoes' },
        (payload) => {
          logger.debug('Rescisão excluída:', payload);
          loadRescisoes();
        }
      )
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'omnia_rescisao_comments' },
        (payload) => {
      ...
      .on(
        'postgres_changes',
        { event: '*', schema: 'public', table: 'omnia_rescisao_attachments' },
        (payload) => {
  ...
  const handleView = (id: string | number) => {
    router.push(`/rescisoes/${id}`);
  };

  const handleDelete = (id: string | number) => {
    const task = rescisoes.find(t => t.id === String(id));
    if (task) {
  ...
  const handleDeleteConfirm = async () => {
    if (taskToDelete) {
      await deleteRescisao(taskToDelete.id);
  ...
  const handleStatusChange = async (id: string | number, statusId: string) => {
    try {
      await updateRescisao(id.toString(), { statusId });
      loadRescisoes();
    } catch (error) {
  ...
  const handleResponsibleChange = async (id: string | number, userId: string) => {
    try {
      const task = rescisoes.find(t => t.id === id.toString());
      const selectedUser = secretarios.find(user => user.id === userId);
      
      if (selectedUser) {
        if (task?.isPrivate) {
          logger.warn('Não é possível alterar responsável de rescisões privadas');
          return;
        }
        
        await updateRescisao(id.toString(), { assignedTo: selectedUser });
        loadRescisoes();
      }
    } catch (error) {
  ...
  const handleDueDateSave = async (newDate: Date | null, taskId: string) => {
    try {
      await updateRescisao(taskId, { dueDate: newDate ?? undefined });
      loadRescisoes();
    } catch (error) {
  ...
  const handlePriorityChange = async (id: string | number, newPriority: string) => {
    try {
      await updateRescisao(id.toString(), { priority: newPriority as RescisaoPrioridade });
      loadRescisoes();
    } catch (error) {
  ...
  const tableData = filteredRescisoes.map(tarefa => {
  ...
  const sortedData = [...tableData].sort((a, b) => {
    ...
    const aOriginalTask = filteredRescisoes.find(t => t.id === a.id);
    const bOriginalTask = filteredRescisoes.find(t => t.id === b.id);
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <Button 
            onClick={() => router.push('/rescisoes/new')}
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Tem certeza que deseja excluir a rescisão "{taskToDelete?.title}"?
              <br />
