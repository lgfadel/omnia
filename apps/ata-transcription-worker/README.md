...
3. Configure, somente no Railway, as variáveis `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` e `OPENAI_API_KEY`.

O worker usa `gpt-4o-transcribe-diarize` com idioma `pt`, divide gravações em blocos de 20 minutos e remove o objeto privado do Storage após concluir. Em caso de falha, mantém o áudio para uma nova tentativa.
